...
@export_range(1, 32) var min_octave_count : int = 10

## How many layers of noise to sum. More octaves give more detail with diminishing returns.
@export_range(1, 32) var max_octave_count : int = 14

## Distance at which we want the max_octaves to be at
@export var max_octave_distance : float = 50.0

## Distance at which we want the min_octaves to be at
@export var min_octave_distance : float = 300.0
...
func _render_callback(_effect_callback_type : int, render_data : RenderData):
	...
	buffer.push_back(min_octave_count)
	...
	buffer.push_back(max_octave_distance)
	buffer.push_back(1.0)
	buffer.push_back(1.0)
...
const source_utils = "
		...
		vec3 fbm(vec2 pos, float octaves) {
...
const source_vertex = "
		...
		void main() {
			...
			float octave_dist_frac_clamped = clamp(octave_dist_frac, 0.0, 1.0);
			float octaves = mix(_MaxOctaves, _MinOctaves, octave_dist_frac_clamped);
			...
			vec3 n = fbm(noise_pos.xz, octaves);
